...
if (TESTING_WARP || TEST_THREAD(0,5) || TEST_THREAD(1,2) ) {
  if (TEST_THREAD(0,5)) {
    // Work-item 0 in workgroup 5:
    test_barrier(2, &(ga[3]));
    atomic_store_explicit(&ga[0], 2, memory_order_release, memory_scope_work_group);
    ...
    test_barrier(2, &(ga[3]));
    atomic_store_explicit(&ga[1], 2, memory_order_release, memory_scope_work_group);
    atomic_store_explicit(&ga[0], 1, memory_order_release, memory_scope_work_group);
    int tmp1 = atomic_load_explicit(&ga[0], memory_order_acquire, memory_scope_work_group);
    out[1] = tmp1;
    }
}
else {
...
__kernel void check_outputs(__global atomic_uint *ga, __global int *out, __global int *result) {
    if (get_global_id(0) == 0) {
      if (out[0] == 2 && out[1] == 2 && ga[0] == 2 && ga[1] == 2) {
        *result=1;
      }
      else {
        *result=0;
