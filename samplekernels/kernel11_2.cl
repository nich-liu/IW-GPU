...
if (TESTING_WARP || TEST_THREAD(0,3) || TEST_THREAD(1,2) ) {
  if (TEST_THREAD(0,3)) {
    // Work-item 0 in workgroup 3:
    test_barrier(2, &(ga[3]));
    atomic_store_explicit(&ga[0], 1, memory_order_seq_cst, memory_scope_work_group);
    ...
    int tmp1 = atomic_load_explicit(&ga[0], memory_order_seq_cst, memory_scope_work_group);
    out[1] = tmp1;
    }
}
else {
...
__kernel void check_outputs(__global atomic_uint *ga, __global int *out, __global int *result) {
    if (get_global_id(0) == 0) {
      if (out[0] == 0 && out[1] == 0 && ga[0] == 1 && ga[1] == 1) {
        *result=1;
      }
      else {
        *result=0;
