...
if (TESTING_WARP || TEST_THREAD(0,3) || TEST_THREAD(1,1) ) {
  if (TEST_THREAD(0,3)) {
    // Work-item 0 in workgroup 3:
    test_barrier(2, &(ga[3]));
    atomic_store_explicit(&ga[1], 2, memory_order_seq_cst, memory_scope_work_group);
    int tmp0 = atomic_load_explicit(&ga[0], memory_order_seq_cst, memory_scope_work_group);
    out[0] = tmp0;
    }
