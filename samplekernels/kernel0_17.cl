...
if (TESTING_WARP || TEST_THREAD(0,0) || TEST_THREAD(1,1) || TEST_THREAD(2,1) ) {
  if (TEST_THREAD(0,0)) {
    // Work-item 0 in workgroup 0:
    test_barrier(3, &(ga[3]));
    int tmp0 = atomic_load_explicit(&ga[1], memory_order_acquire, memory_scope_device);
    if (tmp0 == 1)
    ...
    atomic_store_explicit(&ga[1], 1, memory_order_release, memory_scope_device);
    out[1] = tmp1;
    }
 else if (TEST_THREAD(2,1)) {
    // Work-item 2 in workgroup 1:
    test_barrier(3, &(ga[3]));
    atomic_store_explicit(&ga[0], 2, memory_order_relaxed, memory_scope_device);
    }
}
else {
    // Stress
...
__kernel void check_outputs(__global atomic_uint *ga, __global int *out, __global int *result) {
    if (get_global_id(0) == 0) {
      if (out[0] == 1 && out[1] == 2 && ga[0] == 2 && ga[1] == 1) {
        *result=1;
      }
      else {
        *result=0;
