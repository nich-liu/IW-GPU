...
if (TESTING_WARP || TEST_THREAD(0,0) || TEST_THREAD(1,1) ) {
  if (TEST_THREAD(0,0)) {
    // Work-item 0 in workgroup 0:
    test_barrier(2, &(ga[3]));
    int tmp0 = atomic_load_explicit(&ga[1], memory_order_relaxed, memory_scope_work_group);
...
__kernel void check_outputs(__global atomic_uint *ga, __global int *out, __global int *result) {
    if (get_global_id(0) == 0) {
      if (out[0] == 1 && out[1] == 1 && ga[0] == 1 && ga[1] == 1) {
        *result=1;
      }
      else {
        *result=0;
